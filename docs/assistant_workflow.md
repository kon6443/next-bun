...
- 변경 영향이 크거나 배포 이슈가 의심되면 `bun run build`로 빌드 테스트를 수행한다.
...
- NextAuth 관련 규칙은 `.claude/rules/nextauth.md`를 따른다 (해당 파일을 읽으면 자동 로드).
- UI/UX 관련 규칙은 `.claude/rules/ui.md`를 따른다 (`.tsx`·`.css`를 읽으면 자동 로드).
