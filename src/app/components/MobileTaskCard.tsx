...
// 상태별 라벨 및 색상
const statusConfig: Record<number, { label: string; prevLabel: string; nextLabel: string }> = {
  1: { label: 'Ideation', prevLabel: '', nextLabel: 'Progress' },
  2: { label: 'In Progress', prevLabel: 'Ideation', nextLabel: 'Done' },
  3: { label: 'Completed', prevLabel: 'Progress', nextLabel: '' },
};
...
export function MobileTaskCard({ task, onStatusChange, teamId }: MobileTaskCardProps) {
  ...
  const canGoPrev = taskStatus > 1;
  const canGoNext = taskStatus < 3;
  const config = statusConfig[taskStatus] || statusConfig[1];
    ...
    >
      ...
      <div className="mt-4 pt-3 border-t border-white/5">
        <div className="flex items-center gap-2">
          {/* 이전 상태로 버튼 */}
          <button
            onClick={(e) => canGoPrev && handleStatusChange(taskStatus - 1, e)}
          ...
          {/* 다음 상태로 버튼 */}
          <button
            onClick={(e) => canGoNext && handleStatusChange(taskStatus + 1, e)}
